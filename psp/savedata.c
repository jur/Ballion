...
static void DrawStuff(void)
{
	...
	sceGumLoadIdentity();
	{
		...
		ScePspFVector3 rot =
			{ val * 0.79f * (M_PI / 180.0f), val * 0.98f * (M_PI / 180.0f),
...
char key[] = "RTEA0Q8KJSAJSWET";	// Key to encrypt or decrypt savedata

void initSavedata(SceUtilitySavedataParam * savedata, int mode, char *buffer,
	int size, const char *details)
{
	...
	strcpy(savedata->gameName, "BALLION18");	// First part of the save name, game identifier name
	...
	savedata->dataBuf = buffer;
	savedata->dataBufSize = size;
	savedata->dataSize = size;

	// Set some data
	if (mode == PSP_UTILITY_SAVEDATA_LISTSAVE) {
		strcpy(savedata->sfoParam.title, "Ballion");
		strcpy(savedata->sfoParam.savedataTitle, "Save Game");
		if (details != NULL) {
		...
		entry = rom_getFile("resources/savegame.png");
		if (entry != NULL) {
			savedata->icon0FileData.buf = entry->start;
			savedata->icon0FileData.bufSize = entry->size;
			savedata->icon0FileData.size = entry->size;
		} else {
			printf("Error: Icon missing.\n");
		}
...
static int ShowSaveDialog(int mode, char *buffer, int size, const char *details)
{
	SceUtilitySavedataParam dialog;

	initSavedata(&dialog, mode, buffer, size, details);
...
int savegame_save(char *buffer, int size, const char *details)
{
	...
	SetupGu();
#endif
	rv = ShowSaveDialog(PSP_UTILITY_SAVEDATA_LISTSAVE, buffer, size, details);
#if 0
	sceGuStart(GU_DIRECT,list);
...
int savegame_load(char *buffer, int size)
{
	...
	SetupGu();
#endif
	rv = ShowSaveDialog(PSP_UTILITY_SAVEDATA_LISTLOAD, buffer, size, NULL);
#if 0
	sceGuStart(GU_DIRECT,list);
	sceGuDisable(GU_DEPTH_TEST);
	sceGuFinish();
